...
void
handle_output_redir(char *out_file)
{
...
void
handle_input_redir(char *in_file)
{
	if (strlen(in_file) == 0)
		return;
...
void
handle_error_redir(char *err_file)
{
...
void
handle_left_child(struct pipecmd *p, int fds[2])
{
	int left_fork = fork();
	if (left_fork < 0) {
...
void
handle_right_child(struct pipecmd *p, int fds[2])
{
	int right_fork = fork();
	if (right_fork < 0) {
		perror("error en el fork");
