...
char *
non_canonical_read_line(char *prompt)
{
	char c;
	line_pos = 0;
	int history_pos = 0;
	...
	while (true) {
		...
		if (c == CHAR_ESC) {
			...
			if (esc_seq == '1') {
				if (read(STDIN_FILENO, &esc_seq, 1) < 0) {
					perror("Error al intetar leer de "
					       "STDIN");
					exit(EXIT_FAILURE);
				}
				if (read(STDIN_FILENO, &esc_seq, 1) < 0) {
...
void
write_command_from_history(FILE *histfile)
{
